...
namespace Exoa.Designer
{
    public class Join : MonoBehaviour
    {
        ...
        {
            //Debug.Log(wallMask + " " + obj + " " + mousePos + " " + wallDetectionRadius + " " + angleToAvoidWall);
            ...
            if (selected.point != Vector3.zero)
            {
                ...
                secondRay.origin = obj.transform.position;
                secondRay.direction = Vector3.down;
                RaycastHit[] secondHit = Physics.RaycastAll(secondRay, 100, Layers.ModuleMask).OrderByDescending(hit2 => hit2.distance).ToArray();
                if (secondHit.Length > 0)
                {
                    if (secondHit[0].transform.tag == "Floor")
                    {
                        obj.transform.position = selected.point.SetY(0.3f) - gap;
                    }
                }*/
