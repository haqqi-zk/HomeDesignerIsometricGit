    ...
    public class UISaving : GenericLeftMenu
    {
        ...
        override public void Start()
        {
            ...
            else if (startAction == StartAction.OpenWizard && wizardPopup != null)
            {
                ...
                {
                    LoadInternal("BaseScene", false, true);
        ...
        protected async void OnLevelListChanged(List<string> list)
        {
            ...
            foreach (string name in list)
            {
                ...
                {
                    t2 = await ThumbnailGeneratorUtils.Load(buildingViewName);
                }
                else
                {
                    t2 = await ThumbnailGeneratorUtils.Load(perspViewName);
                }
        ...
        virtual public void LoadInternal(string name, bool sendLoadedEvent = true, bool useBaseScene = false)
        {
            //print("UISaving LoadInternal");
            GameEditorEvents.OnRequestClearAll?.Invoke(clearFloorsUI: true, clearFloorMapUI: true, clearScene: true);
            if (useBaseScene)
            {
                SaveSystem.Create(SaveSystem.Mode.RESOURCES).LoadFileItem(name, dataSerializer.GetFolderName(), (string json) =>
                {
            ...
            {
                /*
                lastSaveContent = await _DatabaseController.Instance.GetJsonStrData(dataSerializer.GetFolderName(), name);
                saveNameInputField.text = name;
                dataSerializer.DeserializeToScene(lastSaveContent);
                GameEditorEvents.OnFileLoaded?.Invoke(dataSerializer.GetFileType());*/
                SaveSystem.Create(SaveSystem.Mode.ONLINE).LoadFileItem(name, dataSerializer.GetFolderName(), (string json) =>
                {
                    //print("dataSerializer:" + dataSerializer);
                    lastSaveContent = json;
                    saveNameInputField.text = name;
        ...
        private void OnGUI()
        {
            ...
            if (HDInputs.OpenSaveFolderPressed())
            {
                if (_DatabaseController.connectionState)
                {
                    Process.Start(SaveSystem.Create(SaveSystem.Mode.ONLINE).GetBasePath(null));
                }
                else
                {
        ...
        {
            _DatabaseController.DebugError("Quitting");
            if (NoNeedToSave())
            {
                Application.Quit();
                return;
        ...
        public bool SaveInternal(string name, Action<string> callback = null)
        {
            ...
            if (_DatabaseController.connectionState)
            {
                SaveSystem.Create(SaveSystem.Mode.ONLINE).SaveFileItem(name + ".json", dataSerializer.GetFolderName(), lastSaveContent, callback == null ? OnSaveDone : callback);
            }
            else
            {
        ...
        public async void GetFileList(string arg = null, FileType fileType = FileType.FloorMapFile)
        {
            ...
            if (fileType == FileType.FloorMapFile || fileType == FileType.InteriorFile || fileType == FileType.ScreenshotFile)
            {
                if (_DatabaseController.connectionState)
                {
                    ...
                    SaveSystem.Create(SaveSystem.Mode.ONLINE).ListFileItems(dataSerializer.GetFolderName(), (SaveSystem.FileList l) =>
                    {
