...
namespace Exoa.Designer
{
    public class InteriorDesigner : MonoBehaviour
    {
        ...
        public void Init()
        {
            ...
            InfoPopup.OnClickColor.AddListener(() =>
            {
                ColorObject();
            });
            InfoPopup.onClickMirror.AddListener(MirrorObject);
            InfoPopup.onClickRotate.AddListener(RotateObject);
            //MaterialPopup.OnClickColorButton.AddListener(ColorObject);
        ...
        public GameObject CreateObj(SceneObject so, bool isSceneObject, bool applyScaleMultiplier, Transform parent)
        {
            ...
            _ColorController colorController = lastObject.GetComponent<_ColorController>();
            Color errorColor = new Color(0, 0, 0, 0);
            if(colorController != null && so.colorVariant != errorColor)
            {
                colorController.SetColor(so.colorVariant);
            }
        ...
        private void Update()
        {
            ...
            if (!overUI && currentPrefabOptions == null && isTap)
            {
                ...
                if (Physics.Raycast(camToMouseRay, out hit, 100, roomMask.value))
                {
                    ...
                    {
                        HideObjectOutline(selectedObject);
                    }
                    ...
                    if (m != null)
                    {
                        ...
                        if(selectedObject != null)
                        {
                            ShowObjectOutline(selectedObject);
                        }
                    }
            ...
            if (!overUI && currentPrefabOptions != null)
            {
                ...
                if (hits.Length > 0)
                {
                    ...
                    if (!currentPrefabOptions.isGroundTile && !currentPrefabOptions.isCeilTile)
                    {
                        hhit = hits[0];
                    ...
                    //added
                    RaycastHit[] secondHits;
                    if (currentPrefabOptions.isGroundTile)
                    {
                        secondHits = Physics.RaycastAll(camToMouseRay, 100, modulesMask.value).OrderByDescending(hit2 => hit2.distance).ToArray();
                        if(secondHits.Length > 0)
                        {
                            if (secondHits[0].transform.tag == "Floor")
                            {
                                modifiedHitPoint = secondHits[0].point;
                                modifiedHitPoint.y += 0;
                            }
                        }
                    }
                    ...
                    if (!isRotating && !isTap && !escapePressed)
                    {
                        ...
                        //ghostObject.transform.rotation = Quaternion.Euler(0, 45 * turns, 0) * ghostObject.GetComponent<ModuleController>().GetInitRotation();
                    ...
                    else
                    {
                        ...
                        if (!positionTaken && !altPressed && ghostObject != null)
                        {
                            ...
                            sceneObject.scale = ghostObject.transform.localScale;
                            GameObject realObject = CreateObj(sceneObject, true, true, modulesContainer);

                            _ColorController colorController = realObject.GetComponent<_ColorController>();
                            if (colorController != null)
                            {
                                Color errorColor = new Color(0, 0, 0, 0);
                                if(selectedObjectColor != errorColor)
                                {
                                    colorController.SetColor(selectedObjectColor);
                                    selectedObjectColor = errorColor;
                                }
                            }
            ...
            if (Input.GetKeyDown(KeyCode.LeftArrow))
            {
                if(ghostObject != null)
                {
                    ghostObject.transform.Rotate(0, -45, 0);
                }
            }
        ...
        private void HideObjectOutline(GameObject gameObject)
        {
        ...
        private void ShowObjectOutline(GameObject gameObject)
        {
            Outline outline = gameObject.GetComponent<Outline>();
            if(outline != null)
            {
        ...
        private void CreateGhost(SceneObject sceneObject, Vector3 gridPos)
        {
            ...
            //sceneObject.rotation = new Vector3(0, 45 * turns, 0);
        ...
        private void OnClickMove()
        {
            ...
            _ColorController colorController = selectedObject.GetComponent<_ColorController>();
            if(colorController != null)
            {
                selectedObjectColor = colorController.GetColor();
            }
        ...
        {
            escapePressed = false;
            RotatePopup.Instance.ShowMode(selectedObject.gameObject);
        }
        private void ColorObject(bool isWall = false)
        {
            escapePressed = false;
            PopupColor.Instance.ShowMode(selectedObject.gameObject, isWall);
        }
        private void MirrorObject()
        {
            Vector3 scaleChange = new Vector3(-1, 1, 1);
            selectedObject.transform.localScale = Vector3.Scale(selectedObject.transform.localScale, scaleChange);
        }
    }
}
